...
class SpaceTravelMockApi {
  ...
  static interceptConsoleLogs() {
    ...
    logTypes.forEach((type) => {
      ...
      console[type] = function (...args) {
        ...
        // Optional: keep logs to a maximum of 1000 entries
  ...
  static getLogsFromLocalStorage() {
    try {
      ...
      // Ensure it's parsed and check if it's an array
      const parsedLogs = logs ? JSON.parse(logs) : [];
      return Array.isArray(parsedLogs) ? parsedLogs : []; // If not an array, return an empty array
    } catch (error) {
      console.error("Error parsing appLogs from localStorage:", error);
